...
$(document).ready(function() {
	$("div.liquid-slider-wrapper").removeClass('hidden-slider');
});
...
function determineSlidesShown() {
  ...
  setSlidesShown();
  //$("div.liquid-slider-wrapper").css( "opacity", "100%" );
};

var weAreOnSlideNumber;
function setSlidesShown() {
  $('#slider-id-wrapper').css('max-width', sliderMaxWidth);
  $('#slider-id-wrapper .liquid-slider .panel').css('width', 100 / (sliderObject.panelCount * numberOfSlidesShown) + '%');
  if (transitionSize > 1){
	$('#slider-id-wrapper .liquid-slider .panel.large').css('width', 200 / (sliderObject.panelCount * numberOfSlidesShown) + '%');
  } else {
	$('#slider-id-wrapper .liquid-slider .panel.large').css('width', 100 / (sliderObject.panelCount * numberOfSlidesShown) + '%');
  }
  $('.liquid-slider-wrapper .liquid-slider .panel .panel').css('width', '100%');
...
function loadBaseArray(){
	$("#slider-id-wrapper .liquid-slider .panel-container .slider-id-panel").each(function(i, val) {
...
/*function relocateLarge(){
	...
	//remove the panels -
	$("#slider-id-wrapper .liquid-slider .panel-container .slider-id-panel").each(function() {
		$(this).detach();
	});
	...
	for (var i in panelArray){
		$("#slider-id-wrapper .liquid-slider .panel-container").append(panelArray[i]);
	}
}*/
...
function getColumnCount(){
	columnCount = sliderObject.panelCount;
	$("#slider-id-wrapper .liquid-slider .panel-container .slider-id-panel").each(function() {
		if($(this).hasClass("large")){columnCount++;}
	});
}
...
$('.first.arrow-left').on('click', function() {
...
$('.first.arrow-right').on('click', function() {
  clearTimeout(lsAutoslide);
...
function addTabs(){
	...
	tabString += '">';
	if (pageCount > 1){$("#slider-id-wrapper.liquid-responsive .arrows a i").show()}
	else {$("#slider-id-wrapper.liquid-responsive .arrows a i").hide()};
	...
	tabString += '</ul>';
	if (pageCount == 1){tabString += '<hr class="no-nav">'}
	// each time tab is refreshed replace html string
	$('div.slider-tab-panel.first').html('');
	$('div.slider-tab-panel.first').html(tabString);
	tabListener();
}
addTabs();

function tabListener(){
	$('.slider-tab-panel.first a').click(function() {
	  //event.preventDefault();
	  var goToSlide = $(this).data('goto') - 1;	
	  $('ul#monitor-slider-nav-ul li').removeClass('active');
...
function updateTab(activate){
	$('ul#monitor-slider-nav-ul li').removeClass('active');
	$('ul#monitor-slider-nav-ul li:nth-child(' + activate + ')').addClass('active');
}

// Move on Swipe
/*$(sliderObject.sliderId + ' .panel').swipe({
  fallbackToMouseEvents: false,
  allowPageScroll: "vertical",
  swipe: function(e, input) {
    // Reverse the swipe direction
      var dir = (input === 'left') ? 1  : -1;
      clearTimeout(lsAutoslide);
...
function setSlidesShownSecond() {
  $('#slider-id-2-wrapper .liquid-slider-wrapper').css('max-width', sliderMaxWidth);
  $('#slider-id-2-wrapper .liquid-slider .panel').css('width', 100 / (secondSliderObject.panelCount * numberOfSlidesShownSecond) + '%');
  if (transitionSize > 1){
	$('#slider-id-2-wrapper .liquid-slider .panel.large').css('width', 200 / (secondSliderObject.panelCount * numberOfSlidesShownSecond) + '%');
  } else {
	$('#slider-id-2-wrapper .liquid-slider .panel.large').css('width', 100 / (secondSliderObject.panelCount * numberOfSlidesShownSecond) + '%');
  }
  $('#slider-id-2-wrapper .liquid-slider .panel .panel').css('width', '100%');
  // DP $('#slider-id-2-wrapper .liquid-slider .panel .panel.large').css('width', '100%');
  // Store the first panel so we know where we are (also reset on resize)
...
var secondSliderObject = $.data( $('#slider-id-2')[0], 'liquidSlider');

// load up the basic tabs for the 
/*function loadBaseArraySecond(){
	$("#slider-id-2-wrapper .liquid-slider .panel-container .slider-id-2-panel").each(function(i, val) {
		if($(this).hasClass("large")){
...
/*function relocateLargeSecond(){
	...
	//remove the panels -
	$("#slider-id-2-wrapper .liquid-slider .panel-container .slider-id-2-panel").each(function() {
		$(this).detach();
	});
	// This sorts the panels so that large ones don't hang half off the page -
	function resort(){
	...
	// This rebuilds the set of panels after rearrangement -
	for (var i in panelArray){
		$("#slider-id-2-wrapper .liquid-slider .panel-container").append(panelArray[i]);
	}
}*/
...
function getColumnCountSecond(){
	columnCountSecond = secondSliderObject.panelCount;
	$("#slider-id-2-wrapper .liquid-slider .panel-container .slider-id-2-panel").each(function() {
		if($(this).hasClass("large")){columnCountSecond++;}
	});
}
...
function secondautoslideFn() {
  secondlsAutoslide = setTimeout(function() {
    // Direction is set based on slide position
    $(secondDirection).trigger('click');
    secondautoslideFn();
  }, autoslideDelay);
}
// Call the autoslide if enabled
if (useAutoslideSecond) {
  // You may want to run this on load if you have a lot of images
  //$(window).bind("load", function () {
    secondautoslideFn();
  //}
...
// Move on arrow clicks
$('.second.arrow-left').on('click', function() {
...
$('.second.arrow-right').on('click', function() {
  clearTimeout(secondlsAutoslide);
...
function addTabsSecond(){
	...
	if (pageCount > 1){$("#slider-id-2-wrapper.liquid-responsive .arrows a i").show()}
	else {$("#slider-id-2-wrapper.liquid-responsive .arrows a i").hide()};
	...
	tabString += '</ul>'
	// each time tab is refreshed replace html string:
	$('div.slider-tab-panel.second').html('');
	$('div.slider-tab-panel.second').html(tabString);
	tabListenerSecond();
}
addTabsSecond();

function tabListenerSecond(){
	$('.slider-tab-panel.second a').click(function() {
	  //event.preventDefault();
	  var goToSlide = $(this).data('goto') - 1;
	  $('ul#monitor-slider-nav-ul-second li').removeClass('active');
	  $(this).parent().addClass('active');
	  //console.log(goToSlide);
	  clearTimeout(secondlsAutoslide);
...
function updateTabSecond(activate){
	$('ul#monitor-slider-nav-ul-second li').removeClass('active');
	$('ul#monitor-slider-nav-ul-second li:nth-child(' + activate + ')').addClass('active');
}
