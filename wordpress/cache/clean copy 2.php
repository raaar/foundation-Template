...
function reverie_scripts_and_styles() {
  if (!is_admin()) {
    ...
    wp_register_style( 'edge-css', get_template_directory_uri() . '/css/overrides-edge.css', array(), '' );
	
	//DM - creating global variable '$page_type':
	global $page_type;
	$page_type = get_field('page_type');
	
	// get_field('page_type', $post->ID ) === "overrides_edge" - now using variable to reduce db requests
    if($page_type === "overrides_edge" || $post->post_type == 'the-edge') {
	    wp_enqueue_style('edge-css');
		//echo 'success!';
    }
    
    wp_register_style( 'academy-css', get_template_directory_uri() . '/css/overrides-academy.css', array(), '' );
    if($page_type === "overrides_academy" || $post->post_type == 'academy') {
	    wp_enqueue_style('academy-css');
		//echo 'success!';
    }
    ...
    */
	...
	//DM: Including the tooltip JS
	//wp_register_script( 'tooltip-js', get_template_directory_uri() . '/js/foundation/foundation.tooltips.js', array( 'jquery' ), '', true );
	//wp_enqueue_script( 'tooltip-js' );
	
	// DM: Adding in the custom javascript files for the liquid slider (there are four of them, two are jquery addons):
	wp_register_script( 'jq-easing', get_template_directory_uri() . '/js/jquery.easing.1.3.js', array( 'jquery' ), '', true );
	wp_register_script( 'jq-touch-swipe', get_template_directory_uri() . '/js/jquery.touchSwipe.min.js', array( 'jquery' ), '', true );
	wp_register_script( 'liquid-slider-custom', get_template_directory_uri() . '/js/jquery.liquid-slider-custom.min.js', array( 'jquery' ), '', true );
	wp_register_script( 'liquid-slider', get_template_directory_uri() . '/js/liquid-slider.js', array( 'jquery', 'jq-easing', 'jq-touch-swipe', 'liquid-slider-custom' ), '', true );
	
	//wp_enqueue_script( 'jq-easing' );
	//wp_enqueue_script( 'jq-touch-swipe' );
	//wp_enqueue_script( 'liquid-slider-custom' );
	wp_enqueue_script( 'liquid-slider' );
	
	// DM: Adding in raf's script, also the flexslider and the fonticon style:
	wp_register_script( 'flexslider', get_template_directory_uri() . '/js/jquery.flexslider-min.js', array( 'jquery' ), '', true );	
	//wp_register_script( 'flexslider-load', get_template_directory_uri() . '/js/load_flexslider.js', array( 'jquery', 'flexslider' ), '', true );
	wp_enqueue_script( 'flexslider' );
	// font awesome style sheet
    wp_register_style( 'font-awesome', get_template_directory_uri() . '/font/font-awesome.min.css', array(), '' );
	wp_enqueue_style('font-awesome');
	wp_register_script( 'raf-script', get_template_directory_uri() . '/js/script.min.js', array( 'jquery' ), '', true );
	wp_enqueue_script( 'raf-script' );
